...
DataTime::DataTime(const DataTime& obj) {
	this->day = obj.day;
...
string DataTime::getFuture(unsigned int N) {
	tm* time_out = make_time_struct(this->day + N, this->month, this->year);
	return make_date_string(time_out);
}

string DataTime::getPast(unsigned int N) {
	tm* time_out = make_time_struct(this->day - N, this->month, this->year);
	return make_date_string(time_out);
}

int DataTime::getDifference(DataTime& obj) {
